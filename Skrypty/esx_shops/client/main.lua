	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				while (not HasAnimDictLoaded("mp_am_hold_up")) do Citizen.Wait(0) end
				TaskPlayAnim(PlayerPedId(), "mp_am_hold_up", "purchase_beer_shopkeeper", 8.0, 3.0, -1, 0, 1, false, false, false)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		--local coords = GetEntityCoords(PlayerPedId()) - po cholere kolejny raz brac coordy gracza, jak petla nizej tez je zbiera, niech wrzuca do globala wyzejs
		for k,v in pairs(Config.Zones) do
			for i = 1, #v.Pos, 1 do
				if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) < Config.DrawDistance and v.ShowMarker) then
