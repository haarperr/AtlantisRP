					...
					case 'NENTER' : {
						...
						if (typeof focused != 'undefined') {
							...
							switch(elem.type) {
								...
								case 'slider': {
									$.post('http://esx_menu_default/menu_input', JSON.stringify({status: "open"}));
						
									$('#menu_' + focused.namespace + '_' + focused.name).find('.menu-item.selected').html("<input type='textbox' style='border: 0px; background: none;'>");
									$('input').focus();
									//console.log("test2");
		...
		if (typeof focused != 'undefined') {
			...
			switch(elem.type) {
				...
				case 'slider': {
					...
					if (typeof elem.options != 'undefined' && num2 < elem.options.length - 1) {
						elem.value = num2;
						//console.log("zmiana na 1 " + num2);
						ESX_MENU.change(focused.namespace, focused.name, elem);
						
					}
					let min = (typeof elem.min == 'undefined') ? 0 : elem.min;
					if (typeof elem.max != 'undefined' && num2 < elem.max && num2 > min) {
						elem.value = num2;
						//console.log("zmiana na 2 " + num2 + "elem: " + elem);
						ESX_MENU.change(focused.namespace, focused.name, elem);
					}
					
					
					$.post('http://' + ESX_MENU.ResourceName + '/menu_input', JSON.stringify({
							status: "notopen"
						}));
