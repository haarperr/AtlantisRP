...
function ShowNoPhoneWarning () 
  ...
  ESX.ShowNotification("Nie posiadasz ~r~telefonu~s~")
end
...
Citizen.CreateThread(function()
  ...
  while true do
      ...
      hasPhone(function (hasPhone)
        if hasPhone == true then 
          if not IsPedSwimming(PlayerPedId()) then
            ...
            ESX.ShowNotification("Nie możesz używać ~r~telefonu~s~ w wodzie")
          end
        else
          ShowNoPhoneWarning()
        end
	...
	if menuIsOpen == true then
		...
		if not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_in', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_listen_base', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_out', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_to_call', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_out', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_to_text', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_in', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_call_in', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_out', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_to_call', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_horizontal_exit', 3)
		and not IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_call_to_text', 3)
		then
			TaskPlayAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_in', 3.0, -1, -1, 50, 0, false, false, false)
		end
    end
	if not menuIsOpen then
		if IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_in', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_listen_base', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_out', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_to_call', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_out', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'cellphone@', 'cellphone_call_to_text', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_in', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_call_in', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_out', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_text_to_call', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_horizontal_exit', 3)
		or IsEntityPlayingAnim(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_call_to_text', 3)
		then
			--Wait(1500)
			PhonePlayOut()
...
AddEventHandler("gcPhone:receiveMessage", function(message, isAnon)
  -- SendNUIMessage({event = 'updateMessages', messages = messages})
  SendNUIMessage({event = 'newMessage', message = message})
  if isAnon then
      message.transmitter = "Anon"
      message.display = "Anon"
    end
...
-- and USE_RTC == false
  if inCall == false then
    inCall = true
    --NetworkSetVoiceChannel(infoCall.id + 1)   -- zakomentowane dla webrtc
 --   NetworkSetTalkerProximity(1.00)
--    Wait(500)
    SendNUIMessage({event = "speak", speak = false})        
   
    NetworkSetTalkerProximity(3.0)  -- zmienione z 0.0 - dla webrtc
  end
  if menuIsOpen == false then
    TooglePhone()
  end
...
RegisterNUICallback('sendMessage', function(data, cb)
  ...
  if data.message == '%pos%' then
    ...
    data.message = 'Problem z nawiązaniem połączenia do modułu GPS, nie udało się pobrać koordynatów.'
  end
  if string.starts(data.message, "#") then
        isAnon = true
  end
  TriggerServerEvent('gcPhone:sendMessage', data.phoneNumber, data.message, isAnon)
end)
