...
function getPhoneRandomNumber()
    local numBase0 = math.random(500,999)
    local numBase1 = math.random(500,999)
...
function _internalAddMessage(transmitter, receiver, message, owner, isAnon)
    TriggerEvent('esx:smsMsg', receiver, transmitter , message)
    if isAnon then
        transmitter = "Anonim"
    end
...
function addMessage(source, identifier, phone_number, message,isAnon)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number)
    local myPhone = getNumberPhone(identifier)
    if otherIdentifier ~= nil then 
        local tomess = _internalAddMessage(myPhone, phone_number, message, 0,isAnon)
        getSourceFromIdentifier(otherIdentifier, function (osou)
            if tonumber(osou) ~= nil then 
                -- TriggerClientEvent("gcPhone:allMessage", osou, getMessages(otherIdentifier))
                TriggerClientEvent("gcPhone:receiveMessage", tonumber(osou), tomess,isAnon)
            end
        end) 
    end
    local memess = _internalAddMessage(phone_number, myPhone, message, 1,isAnon)
    TriggerClientEvent("gcPhone:receiveMessage", sourcePlayer, memess,isAnon)
end
...
AddEventHandler('gcPhone:sendMessage', function(phoneNumber, message,isAnon)
    local _phoneNumber = phoneNumber
    local sourcePlayer = tonumber(source)
    local identifier = getPlayerID(source)
    addMessage(sourcePlayer, identifier, _phoneNumber, message,isAnon)
end)
...
local xp =
    ...
    -- dzwoniacy slyszy odbierajacy z telefonem slyszy
    ...
    -- dzwoniacy slyszy odbierajacy bez telefonu nie slyszy
    if is_valid == true then
            getSourceFromIdentifier(destPlayer, function (srcTo)
                if srcTo ~= nil then
                    AppelsEnCours[indexCall].receiver_src = srcTo
...
ESX.RegisterServerCallback('gcPhone:checkPhoneCount', function(source, cb, target)
    local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
    local mam5telefonow = xPlayer.getInventoryItem('phone').count
    cb(mam5telefonow, source)
end)