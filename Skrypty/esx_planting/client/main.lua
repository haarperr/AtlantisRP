...
AddEventHandler("esx_receptury:RequestStart", function(item_name, time)
  ...
  for k, Cords in pairs(currentItem.cords) do
      if IsPedInAnyVehicle(playerPed) then
				exports.pNotify:SendNotification({text = _U('vehicle_deny'), type = "atlantis", queue = "global", timeout = 2000, layout = "atlantis"})
        do return end
      end
			if isActionStarted then
				exports.pNotify:SendNotification({text = _U('wait_end'), type = "atlantis", queue = "global", timeout = 2000, layout = "atlantis"})
        do return end
			end
      if GetDistanceBetweenCoords (Cords.x, Cords.y, Cords.z, GetEntityCoords(playerPed)) <= Cords.distance then
          ...
          TriggerServerEvent('esx_receptury:RemoveItem', item_name)
					exports.pNotify:SendNotification({text = currentItem.start_msg, type = "atlantis", queue = "global", timeout = 2000, layout = "atlantis"})
  ...
  if not isInLocation then
    exports.pNotify:SendNotification({text = _U('wrong_place'), type = "atlantis", queue = "global", timeout = 2000, layout = "atlantis"})
  else
...
AddEventHandler("esx_receptury:StartAction", function(currentStep, currentAction)
  Citizen.CreateThread(function()
	  ...
	  if IsControlJustPressed(1, Keys["X"]) then
	      ...
	      if data.current.value == currentAction.correct then
	        ...
	        --Show failed object, when player select wrong option
					...
					local playerPed = GetPlayerPed(-1)
					FreezeEntityPosition(playerPed, false)
					exports.pNotify:SendNotification({text = currentItem.fail_msg, type = "atlantis", queue = "global", timeout = 2500, layout = "atlantis"})
	        spawnEndObject(currentItem.object, currentItem.end_object, x, y, z)
					TriggerEvent('esx_receptury:storeStep', currentStep)
	      end
			end
   ...
   --Function when player 'close' ui, but i realy dont even know how it work, bcs he work also when player select option...
	 function(data, menu)
		exports.pNotify:SendNotification({text = _U('doing_action'), type = "atlantis", queue = "global", timeout = 7500, layout = "atlantis"})
		exports.pNotify:SendNotification({text = _U('how_cancel'), type = "atlantis", queue = "global", timeout = 12000, layout = "atlantis"})
