...
function OpenCloakroom()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cesspool_cloakroom',
	{
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				TriggerServerEvent('esx_society:removeVehicleFromGarage', 'cesspool', vehicleProps)
...
function DeleteJobVehicle()
	local playerPed = PlayerPedId()
		...
		if IsInAuthorizedVehicle() then
			...
			TriggerServerEvent('esx_society:putVehicleInGarage', 'cesspool', vehicleProps)
			ESX.Game.DeleteVehicle(CurrentActionData.vehicle)

			--if Config.MaxInService ~= -1 then
			--	TriggerServerEvent('esx_service:disableService', 'cesspool')
			--end
		else
			ESX.ShowNotification(_U('only_cesspool'))
		end
end

function OpenCesspoolActionsMenu()
	local elements = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cesspool_actions',
	{
		title    = 'CesspoolJob',
		align    = 'top-left',
		elements = elements
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'cesspool', function(data, menu)
				menu.close()
			end)
		end
		...
		CurrentAction     = 'cesspool_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end)
end

--function OpenMobileCesspoolActionsMenu()
--	ESX.UI.Menu.CloseAll()
--
--	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_cesspool_actions',
--	{
--		title    = 'Taxi',
--		align    = 'top-left',
--		elements = {
...
--				StopTaxiJob()
--			else
--				if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cesspool' then
--					local playerPed = PlayerPedId()
--					local vehicle   = GetVehiclePedIsIn(playerPed, false)
--
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_cesspool:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_cesspool:getStockItem', itemName, count)
					Citizen.Wait(1000)
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_cesspool:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					-- todo: refresh on callback
					TriggerServerEvent('esx_cesspool:putStockItems', itemName, count)
					Citizen.Wait(1000)
					OpenPutStocksMenu()
				end
			end, function(data2, menu2)
...
AddEventHandler('esx_cesspool:hasEnteredMarker', function(zone)
	...
	elseif zone == 'CesspoolActions' then
		CurrentAction     = 'cesspool_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
...
AddEventHandler('esx_cesspool:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if cesspoolBlip == false then
			if ESX.PlayerData.job and ESX.PlayerData.job.name == 'cesspool' then
					...
					AddTextComponentSubstringPlayerName(_U('blip_cesspool'))
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'cesspool' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_cesspool:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_cesspool:hasExitedMarker', LastZone)
			end
		else
			Citizen.Wait(1000)
		end
...
function WorkJob1()
	...
	animacja()
	procent(500)
	TriggerServerEvent('cesspool:job1')
end
...
function WorkJob2()
	...
	animacja()
	procent(200)
	TriggerServerEvent('cesspool:job2', 'cement')
end
...
function WorkJob3()
	...
	animacja()
	procent(110)
	TriggerServerEvent('cesspool:job3', 'weed')
	animacjanoszenia()
	isHolding = true
end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction and not IsDead then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'cesspool' then
				if CurrentAction == 'cesspool_actions_menu' then
					OpenCesspoolActionsMenu()
		...
		if IsControlJustReleased(0, Keys['F6']) and IsInputDisabled(0) and not IsDead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == 'cesspool' then
			OpenMobileCesspoolActionsMenu()
		end
	end
end)
    ...
    if isHolding == true then
		local vehicle = GetClosestVehicleToPlayer()
			if vehicle ~= 0 then
				...
				local vehicleModel = GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), true))
					if vehicleModel == GetHashKey('cesspool') then
						carload()
						isHolding = false
					else
