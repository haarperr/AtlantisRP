...
function SelectRandomJob()
	local index = GetRandomIntInRange(1,  #Config.JobLocations)

	for k,v in pairs(Config.Zones) do
		if v.Pos.x == Config.JobLocations[index].x and v.Pos.y == Config.JobLocations[index].y and v.Pos.z == Config.JobLocations[index].z then
			return k
		end
	end
end
...
function StartJob()
	...
	TargetRandomJob = SelectRandomJob()
	local zone       = Config.Zones[TargetRandomJob]
	print(zone)
	print(TargetRandomJob)
	Blips['TargetRandomJob'] = AddBlipForCoord(zone.Pos.x,  zone.Pos.y,  zone.Pos.z)
	SetBlipRoute(Blips['TargetRandomJob'], true)

	ESX.ShowNotification("Jedź do punktu")
end

function StopJob()
	--if Blips['TargetRandomJob'] then
		RemoveBlip(Blips['TargetRandomJob'])
		Blips['TargetRandomJob'] = nil
		TargetRandomJob    = nil
		NPCOnJob           = false
		local deliver = AddBlipForCoord(VehicleDeleter.Pos.x, VehicleDeleter.Pos.y, VehicleDeleter.Pos.z)
		print(deliver)
	--end
	...
	--if cancel then
	--	ESX.ShowNotification("Misja zakończona, jedź oddaj samochód")
	--	local deliver = AddBlipForCoord(VehicleDeleter.Pos.x, VehicleDeleter.Pos.y, VehicleDeleter.Pos.z)
	--end
end
...
function OpenCloakroom()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'curier_cloakroom',
	{
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				TriggerServerEvent('esx_society:removeVehicleFromGarage', 'curier', vehicleProps)
...
function DeleteJobVehicle()
	local playerPed = PlayerPedId()
		...
		if IsInAuthorizedVehicle() then
			...
			TriggerServerEvent('esx_society:putVehicleInGarage', 'curier', vehicleProps)
			ESX.Game.DeleteVehicle(CurrentActionData.vehicle)

			--if Config.MaxInService ~= -1 then
			--	TriggerServerEvent('esx_service:disableService', 'curier')
			--end
		else
			ESX.ShowNotification(_U('only_curier'))
		end
end

function OpenCurierActionsMenu()
	local elements = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'curier_actions',
	{
		title    = 'CurierJob',
		align    = 'top-right',
		elements = elements
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'curier', function(data, menu)
				menu.close()
			end)
		end
		...
		CurrentAction     = 'curier_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end)
end

--function OpenMobileCurierActionsMenu()
--	ESX.UI.Menu.CloseAll()
--
--	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_curier_actions',
--	{
--		title    = 'Taxi',
--		align    = 'top-right',
--		elements = {
...
--				StopTaxiJob()
--			else
--				if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'curier' then
--					local playerPed = PlayerPedId()
--					local vehicle   = GetVehiclePedIsIn(playerPed, false)
--
...
function TakeBox()
	if IsInAuthorizedVehicle() then
		 TriggerServerEvent('give:box')
		StartJob()
		
		
	else
		ESX.ShowNotification(_U('only_curier_box'))
	end
end

function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_curier:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_curier:getStockItem', itemName, count)
					Citizen.Wait(1000)
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_curier:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					-- todo: refresh on callback
					TriggerServerEvent('esx_curier:putStockItems', itemName, count)
					Citizen.Wait(1000)
					OpenPutStocksMenu()
				end
			end, function(data2, menu2)
...
AddEventHandler('esx_curier:hasEnteredMarker', function(zone)
	...
	elseif zone == 'CurierActions' then
		CurrentAction     = 'curier_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	...
	elseif zone == 'BuyVehicle' then
		...
		elseif zone == 'TakeBoxes' then
		CurrentAction     = 'take_box'
		CurrentActionMsg  = _U('press_to_takebox')
		CurrentActionData = {}
		...
		elseif zone == 'Job5' then
		CurrentAction     = 'Job5'
		CurrentActionMsg  = _U('press_to_work')
		CurrentActionData = {}
		
		elseif zone == 'Job6' then
		CurrentAction     = 'Job6'
		CurrentActionMsg  = _U('press_to_work')
		CurrentActionData = {}
		
		elseif zone == 'Job7' then
		CurrentAction     = 'Job7'
		CurrentActionMsg  = _U('press_to_work')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_curier:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if curierBlip == false then
			if ESX.PlayerData.job and ESX.PlayerData.job.name == 'curier' then
					...
					AddTextComponentSubstringPlayerName(_U('blip_curier'))
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'curier' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_curier:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_curier:hasExitedMarker', LastZone)
			end
		else
			Citizen.Wait(1000)
		end
...
function WorkJob1()
	ESX.ShowNotification("DEBUG: PRACA 1")
	TriggerServerEvent('curier:job1')
end

function WorkJob2()
	ESX.ShowNotification("DEBUG: PRACA 2")
	TriggerServerEvent('curier:job2')
end

function WorkJob3()
	ESX.ShowNotification("DEBUG: PRACA 3")
	TriggerServerEvent('curier:job3')
end

function WorkJob4()
	ESX.ShowNotification("DEBUG: PRACA 4")
	TriggerServerEvent('curier:job4')
end

function WorkJob5()
	ESX.ShowNotification("DEBUG: PRACA 5")
	TriggerServerEvent('curier:job5')
end

function WorkJob6()
	ESX.ShowNotification("DEBUG: PRACA 6")
	TriggerServerEvent('curier:job6')
end

function WorkJob7()
	ESX.ShowNotification("DEBUG: PRACA 7")
	TriggerServerEvent('curier:job7')
end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction and not IsDead then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'curier' then
				if CurrentAction == 'curier_actions_menu' then
					OpenCurierActionsMenu()
				...
				elseif CurrentAction == 'buy_vehicle' then
					OpenShopMenu()
				elseif CurrentAction == 'take_box' then
					TakeBox()
				...
				elseif CurrentAction == 'Job4' then
					WorkJob4()
					StartJob()
				elseif CurrentAction == 'Job5' then
					WorkJob5()
					StartJob()
				elseif CurrentAction == 'Job6' then
					WorkJob6()
					StartJob()
				elseif CurrentAction == 'Job7' then
					WorkJob7()
					StartJob()
				end
			...
			if IsControlJustReleased(3, Keys['DELETE']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'curier' then
				if IsInAuthorizedVehicle() then
			...
			if IsControlJustReleased(0, Keys['F6']) and IsInputDisabled(0) and not IsDead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == 'curier' then
				OpenMobileCurierActionsMenu()
			end
	end
end)
