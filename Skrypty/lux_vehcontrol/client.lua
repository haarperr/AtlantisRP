				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if not IsPauseMenuActive() then
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, 19) then --or IsDisabledControlJustReleased(0, 82) then
...
--							end
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, 80) then --or IsDisabledControlJustReleased(0, 81) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- on
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, 80) then --or IsDisabledControlPressed(0, 81) then
									actv_manu = true
								else
									actv_manu = false
								end
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							elseif IsDisabledControlJustReleased(0, 83) then -- INPUT_VEH_NEXT_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer
							-- IND H
							elseif IsControlJustReleased(0, 213) then -- INPUT_FRONTEND_CANCEL / Home
