...
function K9Menu()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'k9_menu',
    {
        ...
        align    = 'right',
        elements = {
			{label = 'Przywołaj/odwołaj', value = 'call'},
            {label = 'Pojazd: wejdź/wyjdź', value = 'getinout'},
            {label = 'Komenda: Siad!', value = 'sit'},
            {label = 'Komenda: Leżeć!', value = 'lay'},
...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:ToggleK9", function(model)
        if spawned_ped == nil then
            ...
            SetEntityHealth(spawned_ped, 200)
                    ...
                    if door ~= false then
                        if isRestricted then
                            if CheckVehicleRestriction(vehicle, vehList) then
                                TaskFollowToOffsetOfEntity(spawned_ped, vehicle, 0.5, 0.0, 0.0, 5.0, -1, 0.0, 1)
                                local playerCoords = GetEntityCoords(PlayerPedId())
                                local pedCoords = GetEntityCoords(spawned_ped)
