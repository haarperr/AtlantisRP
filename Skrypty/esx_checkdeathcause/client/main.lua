local Keys = {
  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Wciśnij [~b~E~s~] żeby sprawdzić stan.', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Obrażenia zostały zadane w tym miejscu.', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Sprawdź przczynę obrażeń', value = 'deathcause'})
	  table.insert(elements, {label = 'Spróbuj sprawdzić gdzie obrażenia zostały zadane', value = 'damage'})
