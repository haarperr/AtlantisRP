...
Citizen.CreateThread(function ()
    while true do
        ...
        if IsChoosing then
            ...
            timer = timer + 1
            if timer > 60000 then
                TriggerServerEvent("kashactersS:dropMe")
            end
        else 
            timer = 0
        end
...
AddEventHandler('kashactersC:SpawnCharacter', function(spawn)
    ...
    TriggerServerEvent("esx_fixAddon:DataStore")
    TriggerServerEvent("esx_fixAddon:Accounts")
    TriggerServerEvent("esx_fixAddon:Inventory")
    TriggerServerEvent("esx_property:fixServerProperties")
    TriggerServerEvent("esx_fixAddon:DataStore")
    reloadskin() -- zaladuj skina bo cloning plus to kurwa i mixuje ped componenty JD
end)

function reloadskin()
    --SetPlayerCanDoDriveBy(PlayerId(), false)
    SetPlayerHealthRechargeMultiplier(PlayerId(), 0.0)
    SetPedMaxTimeUnderwater(PlayerPedId(), 37.5)
    Wait(30000)
