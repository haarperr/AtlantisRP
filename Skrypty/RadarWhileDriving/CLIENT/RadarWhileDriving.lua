    ...
    while true do
		...
		if IsPedInAnyVehicle(PlayerPedId(), true) then
			if (OnlyDriver and (GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), -1) == PlayerPedId())) or not OnlyDriver then
				DisplayRadar(true)
				--[[
				 if (OnlyDriverFib and (GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), -1) == PlayerPedId())) or not OnlyDriverFib then
					 local vehicle          = GetVehiclePedIsIn(PlayerPedId(),  true)
					 --print(GetEntityModel(vehicle))
					 local HashBaller = GetHashKey("baller6")
					 --print(PlayerData.job.name)
					 if (GetEntityModel(vehicle) == HashBaller and PlayerData.job.name ~= "fib") then
						 
						 ESX.ShowNotification("Brak autoryzacji")
...
--				 print("Health: " .. PlayerHealth)
				...
				--if PlayerHealth < 175 and PlayerHealth > 100 then
					--HealthBarW = (((MM.width / 2) - (BarSpacer / 2)) / 75 * (PlayerHealth - 100) * 0.75)
					HealthBarW = (((MM.width / 2) - (BarSpacer / 2)) / 75 * (PlayerHealth - 100 ) * 0.75)
				--	print(HealthBarW)
